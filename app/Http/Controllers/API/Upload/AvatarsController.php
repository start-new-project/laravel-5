...
class AvatarsController extends Controller
{
    ...
    public $upload_path = "upload/avatars/";
    ...
    public function resize(Request $request){ 
        ...
        $image->resize = true;
        $image->by = auth()->id();
        $image->album = 'avatarexit-'.$user_id;
    ...
    public function save(Request $request){
        $status = false;
        if((!empty($request->user))&&(!empty($request->avatar))){
            $user = User::find($request->user);
            $avatar = MediaFiles::find($request->avatar);
            if(!empty($user->id)&&(!empty($avatar->id))&&($user->canUpload())){
